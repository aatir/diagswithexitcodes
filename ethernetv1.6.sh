...
if [ "$1" = "0" ]
then
	b=`ping -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
	if [ "$a" = "$b" ]
	then
		echo " ETHERNET TEST - PASSED [ETH0 given Passed on ETH0] "
	else 
		c=`ping -I eth1 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
		if [ "$a" = "$c" ]
		then
			echo " ETHERNET TEST - PASSED [ETH0 given Passed on ETH1] " 
			else 
			d=`ping -I eth2 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
			if [ "$a" = "$d" ]
			then
	...
	if [ "$1" = "1" ]
	then
		b=`ping -I eth1 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
		if [ "$a" = "$b" ]
		then
			echo " ETHERNET TEST - PASSED [ETH1 given Passed on ETH1] "
		else 
			c=`ping -I eth2 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
			if [ "$a" = "$c" ]
			then
				echo " ETHERNET TEST - PASSED [ETH1 given Passed on ETH2] " 
				else 
				d=`ping  -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
				if [ "$a" = "$d" ]
				then
		...
		if [ "$1" = "2" ]
			then
				b=`ping -I eth2 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
				if [ "$a" = "$b" ]
				then
					echo " ETHERNET TEST - PASSED [ETH2 given Passed on ETH2] "
				else 
					c=`ping -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
					if [ "$a" = "$c" ]
					then
						echo " ETHERNET TEST - PASSED [ETH2 given Passed on ETH0] " 
					else 
					d=`ping -I eth1 -c 2 -q $(ipaddr) | grep packets | cut -d ] -f 2`
					if [ "$a" = "$d" ]
					then
